...
public final class CronExpression implements Serializable, Cloneable {
	...
	private static final Pattern PAT = Pattern.compile("^L-[0-9]*[W]?");
	...
	protected int storeExpressionVals(int pos, String s, int type) throws ParseException {
		...
		char c = s.charAt(i);
		if ((c >= 'A') && (c <= 'Z') && (!s.equals("L")) && (!s.equals("LW")) && (!PAT.matcher(s).matches())) {
	...
	protected int[] getValue(int v, String s, int i) {
		char c = s.charAt(i);
		...
		val[0] = (i < s.length()) ? i : i + 1;
		val[1] = Integer.parseInt(s1.toString());
		return val;
	}
